...
const Topup = ({ topUpAccount, refetchWallet, balance, sponsored, isIP = false, isCustomCurrMintOption }) => {
	...
	const selectedNetwork = isIP ? storyAeneidTestnet : farcasterStates?.frameData?.selectedNetwork
	const isCustomCurrMint = isCustomCurrMintOption || farcasterStates?.frameData?.isCustomCurrMint
	const TxFeeForDeployment = chain?.id === 137 ? 0.7 : chain_id === morph?.id ? 0.0002 : 0.00009
	const txFeeForMint = isCustomCurrMint ? 0.00005 : 0.00009
	const txFeeForOxSplitCreate = chain?.id === 137 ? 0.7 : chain_id === morph?.id ? 0.0002 : 0.00009
	const isCreatorSponsored = isIP ? true : farcasterStates.frameData.isCreatorSponsored
					...
					{isSufficientBalance ? (
						...
						<>
							...
							<Typography variant="h6" color="blue-gray">
								...
								<>
									{isIP ? storyAeneidTestnet?.name : chain?.name} {isIP ? storyAeneidTestnet?.nativeCurrency?.symbol : chain?.nativeCurrency?.symbol}
								</>
							</Typography>
