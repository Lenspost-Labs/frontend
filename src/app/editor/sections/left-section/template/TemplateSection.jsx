...
import { getAllTemplates, getUserPublicTemplates } from '../../../../../services'
...
import { assetsTrack, fnLoadJsonOnPage, fnLoadMore, replaceImageURL } from '../../../../../utils'
...
const DesignCard = ({ item, id, preview, json, tab, isGated, gatedWith, referredFrom, modal, setModal, assetsRecipientElementData }) => {
	const store = useStore()
	const { referredFromRef, preStoredRecipientDataRef, isMobile, actionType, setOpenBottomBar } = useContext(Context)
		...
		>
			...
			<div className="transition-transform ease-in-out duration-300 relative mb-3">
				...
				<LazyLoadImage
					...
					effect="blur"
					src={tab === 'user' ? replaceImageURL(preview[stPreviewIndex]) : replaceImageURL(preview)}
					alt="Preview Image"
				/>
			</div>
			...
			<div className="h-full overflow-y-scroll">
				...
				{!isMobile && (
					<div>
						...
						<CompCarousel type="background" author="" campaign="christmas" />
							...
							.map((item) => {
								return (
									<DesignCard
										...
										preview={item?.imageLink != null && item?.imageLink.length > 0 && item?.imageLink}
										key={item?.id}
										tab="user"
										modal={modal}
										setModal={setModal}
