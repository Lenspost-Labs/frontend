      ...
      shareOnTwitter({
        ...
        .catch((err) => {
          ...
          if (err?.response?.data?.message?.errors?.length > 0) {
            ...
            toast.error(
              err?.response?.data?.message || err?.err?.response?.data?.error
            );
          }
        });
    } catch (error) {
            ...
            {isAuthChecking ? (
              ...
              <div className="flex py-5 px-5 text-center gap-5 flex-col items-center justify-center">
                <p className="text-sm text-gray-500">
                  You're not logged in to Twitter/X, Please login to share your
                  frame
                </p>
                <Button
