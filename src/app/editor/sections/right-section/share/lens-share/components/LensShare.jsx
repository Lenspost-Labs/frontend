...
import { useAccount, useChainId, useSignMessage, useSwitchChain, useConfig } from 'wagmi'
...
import { getFromLocalStorage, saveToLocalStorage, errorMessage, isEthAddress, isLensHandle } from '../../../../../../../utils'
...
import { CustomPopover, InputBox, InputErrorMsg, NumberInputBox } from '../../../../../common'
...
import { Button, Select, Option, Tabs, Tab, TabsHeader, Alert, Spinner, Typography } from '@material-tailwind/react'
...
const LensShare = () => {
	...
	const formatDateTimeISO8601 = (date, time) => {
		if (!date || !time || date === 'Invalid Date' || time === 'Invalid Date') return
	...
	const monetizationSettings = () => {
		if (!enabled.chargeForCollect && !enabled.limitedEdition && !enabled.timeLimit && !enabled.whoCanCollect) {
			return false
		}
		...
		if (enabled.timeLimit) {
			canvasParams = {
				...canvasParams,
				endTimestamp: formatDateTimeISO8601(enabled.endTimestamp.date, enabled.endTimestamp.time),
			}
		}
	...
	const addRecipientInputBox = () => {
		if (enabled.splitRevenueRecipients.length < 5) {
			setEnabled({
				...enabled,
				splitRevenueRecipients: [...enabled.splitRevenueRecipients, { recipient: '', split: 1.0 }],
			})
		}
	}

	const removeRecipientInputBox = (index) => {
		const updatedRecipients = enabled.splitRevenueRecipients.filter((_, i) => i !== index)
		setEnabled({
			...enabled,
	...
	const splitEvenPercentage = () => {
		const result = enabled.splitRevenueRecipients.map((item) => {
			return {
				...
				split: Math.floor((100 / enabled.splitRevenueRecipients.length).toFixed(2)),
			}
		})
	...
	useEffect(() => {
		if (isAuthenticated) {
			const updatedRecipients = parentRecipientListRef.current.slice(0, 4).map((item) => ({
				recipient: item,
				split: 1.0,
			}))
	...
	return (
		<>
			<div className="flex flex-col ">
				<div className="relative mt-1 pt-2 pb-4 ">
					<div className="">
							...
							<Switch.Group>
								<div className="mb-4">
									...
									<div className="flex justify-between">
										<Switch.Label className="w-4/5 opacity-50">Get paid when someone collects your post</Switch.Label>
										<Switch
											...
											className={`${
												enabled.chargeForCollect ? 'bg-[#e1f16b]' : 'bg-gray-200'
										...
										>
											<span
												className={`${
													enabled.chargeForCollect ? 'translate-x-6' : 'translate-x-1'
									...
									<div className={` ${!enabled.chargeForCollect && 'hidden'}`}>
										<div className="flex gap-5">
											...
											<div className="flex flex-col py-2">
												...
												<Select name="chargeForCollectCurrency" id="chargeForCollectCurrency" label="Currency" value={enabled.chargeForCollectCurrency}>
										...
										{priceError.isError && <InputErrorMsg message={priceError.message} />}
									</div>
								</div>
								...
								<div className={`mb-4 ${!enabled.chargeForCollect && 'hidden'}`}>
									...
									<div className="flex justify-between">
										<Switch.Label className="w-4/5 opacity-60">Share your fee with people who amplify your content</Switch.Label>
										<Switch
											...
											className={`${
												enabled.mirrorReferralReward ? 'bg-[#e1f16b]' : 'bg-gray-200'
											} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
										>
											<span
												className={`${
													enabled.mirrorReferralReward ? 'translate-x-6' : 'translate-x-1'
												} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
											/>
										</Switch>
									</div>
									<div className={`flex ${!enabled.mirrorReferralReward && 'hidden'}`}>
										<div className="flex flex-col w-full py-2">
											...
											/>
											{referralError.isError && <InputErrorMsg message={referralError.message} />}
										</div>
									</div>
								</div>
													...
													<div key={index} className="flex justify-between gap-2 items-center w-full py-2">
														...
														<div className="flex justify-between items-center w-1/3">
															<NumberInputBox
																...
																onChange={(e) => {
																	handleRecipientChange(index, 'split', Number(parseFloat(e.target.value).toFixed(2)))
																}}
															/>
															{!restrictRemoveRecipientInputBox(index, recipient) && (
																<TiDelete className="h-6 w-6 cursor-pointer" color="red" onClick={() => removeRecipientInputBox(index)} />
															)}
														</div>
													</div>
													{index == 0 && (
														<div className="flex flex-row align-middle  text-center justify-start mt-1">
															<span className="italic mt-2 opacity-80">Small fee to support our team!</span>

															{/* This is the custom popover that appears on the sidebar*/}
															<CustomPopover icon="" animationData={animationData} />
														</div>
													)}
												</>
											)
								...
								<div className="mb-4">
									...
									<div className="flex justify-between">
										<Switch.Label className="w-4/5 opacity-60">Make the collects exclusive</Switch.Label>
										<Switch
										...
										>
											<span
												className={`${
													enabled.limitedEdition ? 'translate-x-6' : 'translate-x-1'
												} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
											/>
										</Switch>
									</div>
									<div className={`flex ${!enabled.limitedEdition && 'hidden'}`}>
										<div className="flex flex-col w-full py-2 opacity-60">
											...
											/>
											{editionError.isError && <InputErrorMsg message={editionError.message} />}
										</div>
									</div>
								</div>
								...
								<div className="mb-4">
									...
									<div className="flex justify-between">
										<Switch.Label className="w-4/5 opacity-60">Collect duration</Switch.Label>
										<Switch
                      ...
                      flex flex-col w-full`}
										...
										<div className={`flex flex-col my-2`}>
											...
											<div className="flex flex-row border-l-8 border-l-[#e1f16b] p-4 rounded-md">
												<div className="flex flex-col">
													<div className="text-4xl text-[#E699D9]">{enabled.endTimestamp.date.slice(0, 2)}</div>
													<div className="text-lg text-[#2D346C]">{enabled.endTimestamp.date.slice(2)}</div>
												</div>

												<div className="flex flex-col ml-4">
													<div className="ml-2 mt-10">{enabled.endTimestamp.time}</div>
												</div>
											</div>
										</div>
									</div>
								...
								<div className="mb-4">
									...
									<div className="flex justify-between">
										<Switch.Label className="w-4/5 opacity-60">Only followers can collect</Switch.Label>
										<Switch
										...
										>
											<span
												className={`${
													enabled.whoCanCollect ? 'translate-x-6' : 'translate-x-1'
												} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
											/>
										</Switch>
									</div>
				...
				{!getEVMAuth ? (
					...
					<div className="mx-2 outline-none">
						<Button
							...
							color="red"
						>
							{isLoadingSwitchNetwork ? 'Switching' : 'Switch'} to {chains[0]?.name} Network {isLoadingSwitchNetwork && <Spinner />}
						</Button>
					</div>
