...
export const twitterAuthenticateCallback = async (token, verifier) => {
	try {
		// authenticated request
		const result = await api.post(`${API}/twitter/callback?oauth_token=${token}&oauth_verifier=${verifier}`)
...
// need auth token (jwt)
export const createCanvas = async ({ data, referredFrom, assetsRecipientElementData, preview }) => {
...
// need auth token (jwt)
export const updateCanvas = async ({ id, data, referredFrom, isPublic, assetsRecipientElementData, preview, tags }) => {
...
// need auth token (jwt)
export const shareOnSocials = async ({ canvasData, canvasParams, platform }) => {
...
export const getIsUserWhitelisted = async (walletAddress) => {
	const result = await api.get(`${API}/util/whitelisted?wallet=${walletAddress}`)
...
export const apiGetPolotnoTexts = async () => {
	const result = await api.get(`https://api.polotno.com/api/get-text-templates?KEY=OpcLd8G0QzCfvasc0WkU`)
	return {
		data: result?.data,
	}
}
