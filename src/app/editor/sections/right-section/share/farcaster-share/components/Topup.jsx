...
const Topup = ({ topUpAccount, refetchWallet, balance, sponsored }) => {
	...
	const TxFeeForDeployment = chain?.id === 137 ? 0.7 : 0.00009
	const txFeeForMint = isCustomCurrMint ? 0.00005 : 0.00005
	...
	const payForMintsForCustomCurr = Number(TxFeeForDeployment).toFixed(18).toString()
	const payForMintsForSponsored = Number(txFeeForMint * numberOfExtraMints + TxFeeForDeployment)
		.toFixed(18)
		.toString()
	...
	const payForMints = isCustomCurrMint ? payForMintsForCustomCurr : payForMintsForSponsored
			...
			<List>
				<ListItem className="flex-col items-end gap-2">
					{isSufficientBalance ? (
						...
						<>
							<Typography variant="h6" color="blue-gray">
								Switch chain to {ENVIRONMENT === 'production' ? 'Base' : 'BaseSepolia'} for Topup
							</Typography>
							<Button
								onClick={() =>
									switchChain({
