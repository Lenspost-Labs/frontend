...
import { getFromLocalStorage, errorMessage, addressCrop, saveToLocalStorage, chainLogo, priceFormatter } from '../../../../../../../utils'
import { useAppAuth, useLocalStorage } from '../../../../../../../hooks/app'
import { APP_ETH_ADDRESS, FRAME_URL, LOCAL_STORAGE, TOKEN_LIST, URL_REGEX, isSponsoredChain } from '../../../../../../../data'
import { Avatar, Button, Option, Select, Spinner, Textarea } from '@material-tailwind/react'
...
import { deployZoraContract, getImageByCanvasId, getOrCreateWallet, mintToXchain, postFrame } from '../../../../../../../services/apis/BE-apis'
...
const FarcasterNormalPost = () => {
	...
	const [isDeployingZoraContractError, setIsDeployingZoraContractError] = useState(false)
	const [isDeployingZoraContractSuccess, setIsDeployingZoraContractSuccess] = useState(false)
	...
	const isRecipientAddDuplicate = () => {
		const result = farcasterStates?.frameData?.fcSplitRevenueRecipients.filter((item, index) => {
			return farcasterStates?.frameData?.fcSplitRevenueRecipients.findIndex((item2) => item2.address === item.address) !== index
		})
	...
	const isPercentage100 = () => {
		const result = farcasterStates?.frameData?.fcSplitRevenueRecipients.reduce((acc, item) => acc + item.percentAllocation, 0)
	...
	const sortRecipientsByAddress = (recipients) => {
		return recipients.sort((a, b) =>
			a.address?.toLowerCase()?.localeCompare(b.address.toLowerCase(), undefined, {
				sensitivity: 'base',
			})
		)
	}
	...
	const removeAndAggregateDuplicates = (recipients) => {
		...
		recipients.forEach((recipient) => {
			...
			if (addressMap.has(address)) {
				addressMap.set(address, addressMap.get(address) + recipient.percentAllocation)
			} else {
		...
		return Array.from(addressMap.entries()).map(([address, percentAllocation]) => ({
			address,
			percentAllocation,
		}))
	}
		...
		setFarcasterStates((prevState) => {
			...
			if (name === 'allowedMints') {
				if (!value || value <= 0) {
					...
					newState.frameData.allowedMintsError = 'Please enter a valid number of allowed mints'
				} else {
			...
			if (name === 'customCurrAmount') {
				if (!value || value <= 0.0001) {
					...
					newState.frameData.customCurrAmountError = 'Price should not be less than 0.0001'
				} else {
	...
	const monetizationSettings = () => {
		...
		if (farcasterStates.frameData?.isFrame) {
			...
			if (!farcasterStates?.frameData?.isCreatorSponsored && !farcasterStates?.frameData?.isCustomCurrMint) {
				canvasParams = {
					...canvasParams,
	...
	const sharePost = async (platform) => {
		...
		if (isMobile && actionType === 'composer') {
			...
			const embeds = farcasterStates?.frameData?.isFrame ? FRAME_URL + '/frame/' + frameId : imageUrl
			console.log(embeds)
	...
	const handleSubmit = () => {
		...
		if (farcasterStates.frameData?.isFrame && (farcasterStates.frameData?.allowedMintsIsError || !farcasterStates.frameData?.allowedMints)) {
		...
		// check if external link is a valid URL
		if (farcasterStates.frameData?.isFrame && farcasterStates.frameData?.isExternalLinkError) {
			toast.error('Please enter a valid URL')
			return
		}

		// check if recipient address is same
		if (farcasterStates.frameData?.isFrame && farcasterStates.frameData?.fcSplitRevenueRecipients?.length > 0) {
		...
		if (farcasterStates.frameData?.isFrame && farcasterStates?.frameData?.isCustomCurrMint && !isPercentage100()) {
			toast.error('Total percentage should be 100')
			return
		}

		if (farcasterStates.frameData?.isFrame && farcasterStates.frameData?.isCustomCurrMint && !checkCustomCurrAmt()) {
			return
		}

		if (farcasterStates.frameData?.isFrame && farcasterStates.frameData?.isCustomCurrMint && !farcasterStates.frameData?.selectedNetwork?.id) {
			return toast.error('Please select a network')
		}

		if (farcasterStates.frameData?.isFrame && farcasterStates.frameData?.isCustomCurrMint && !farcasterStates.frameData?.customCurrSymbol) {
		...
		setFarcasterStates((prevState) => {
			...
			if (key === 'percentAllocation' && index === 0) {
				if (value < 10 || value > 100 || isNaN(value)) {
					...
					newState.frameData.fcSplitErrorMsg = 'Platform fee should be between 10% to 100%'
				} else {
				...
				if (value < 1 || value > 100 || isNaN(value)) {
					newState.frameData.isFcSplitError = true
					newState.frameData.fcSplitErrorMsg = 'Split should be between 1% to 100%'
				} else {
					newState.frameData.isFcSplitError = false
					newState.frameData.fcSplitErrorMsg = ''
				}
			...
			const updatedRecipients = [...newState.frameData.fcSplitRevenueRecipients]
	...
	const splitEvenPercentage = () => {
		const result = farcasterStates?.frameData?.fcSplitRevenueRecipients.map((item) => {
			return {
				address: item.address,
				percentAllocation: Math.floor((100 / farcasterStates?.frameData?.fcSplitRevenueRecipients.length).toFixed(2)),
			}
		})
	...
	const addArrlistInputBox = (key) => {
		if (key === 'fcSplitRevenueRecipients') {
			setFarcasterStates({
				...
				frameData: {
					...farcasterStates?.frameData,
					[key]: [...farcasterStates?.frameData[key], { address: '', percentAllocation: '' }],
				},
			})
			return
		}
	...
	useEffect(() => {
		if (isUploadSuccess && farcasterStates.frameData?.isCreatorSponsored) {
			...
			const deployArgs = {
				...
				pricePerToken: priceFormatter(chain?.id, farcasterStates?.frameData?.customCurrAmount),
	...
	useEffect(() => {
		if (isUploadSuccess && !farcasterStates.frameData?.isCreatorSponsored && !farcasterStates.frameData?.isCustomCurrMint) {
			setIsPostingFrame(false)
			writeContract({
	...
	}, [isWriteError, isError, isPostingFrameError, isDeployingZoraContractError, isUploadError])
		...
		<>
			...
			{actionType !== 'composer' && (
				<div className="mb-4 mt-4">
					...
					<div className="w-4/5 opacity-75"> Share your post in the Farcaster channel. </div>
				</div>
			)}
			{actionType === 'composer' && (
				<div className="mb-4 mt-4">
					<div className="flex justify-between">
						...
						<Switch
							...
							className={`${
								!farcasterStates?.frameData.isFrame ? 'bg-[#e1f16b]' : 'bg-gray-200'
							} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
						>
							<span
								className={`${
									!farcasterStates?.frameData.isFrame ? 'translate-x-6' : 'translate-x-1'
								} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
							/>{' '}
						</Switch>
					</div>
					<div className="w-4/5 opacity-75"> Share as a cast on Farcaster. </div>
				</div>
			)}
			...
			<div className="mb-4 mt-4">
				...
				<div className="w-4/5 opacity-75"> Share as Mintable Frame on Farcaster. </div>
			</div>
			...
			<div className={`${!farcasterStates.frameData?.isFrame && 'hidden'} `}>
				<div className="mb-4">
					<div className="flex justify-between">
						...
						<Switch
							...
							className={`${
								farcasterStates.frameData?.isGateWith ? 'bg-[#e1f16b]' : 'bg-gray-200'
							} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
						>
							<span
								className={`${
									farcasterStates.frameData?.isGateWith ? 'translate-x-6' : 'translate-x-1'
								} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
							/>{' '}
						</Switch>
					</div>
					<div className="w-4/5 opacity-75"> Control content access based on engagement. </div>

					<div className={`${!farcasterStates.frameData?.isGateWith && 'hidden'}`}>
						{!isMobile && (
							<>
								<div className="flex justify-between py-2">
									...
									>
										<span
											className={`${
												farcasterStates.frameData?.isLike ? 'translate-x-6' : 'translate-x-1'
											} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
										/>{' '}
									</Switch>
								</div>
								...
								<div className="flex justify-between py-2">
									...
									<Switch
										...
										className={`${
											farcasterStates.frameData?.isRecast ? 'bg-[#e1f16b]' : 'bg-gray-200'
										} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
									>
										<span
											className={`${
												farcasterStates.frameData?.isRecast ? 'translate-x-6' : 'translate-x-1'
											} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
										/>{' '}
									</Switch>
								</div>
								...
								<div className="flex justify-between py-2">
									...
									<Switch
										...
										className={`${
											farcasterStates.frameData?.isFollow ? 'bg-[#e1f16b]' : 'bg-gray-200'
										} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
									>
										<span
											className={`${
												farcasterStates.frameData?.isFollow ? 'translate-x-6' : 'translate-x-1'
											} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
										/>{' '}
									</Switch>
								</div>
						...
						<div className="flex justify-between py-2">
							...
							<Switch
								...
								className={`${
									farcasterStates.frameData?.isChannel ? 'bg-[#e1f16b]' : 'bg-gray-200'
								} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
							>
								<span
									className={`${
										farcasterStates.frameData?.isChannel ? 'translate-x-6' : 'translate-x-1'
									} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
								/>{' '}
							</Switch>
						</div>
							...
							<>
								<div className="flex justify-between py-2">
									...
									<Switch
										...
										className={`${
											farcasterStates.frameData?.isCollection ? 'bg-[#e1f16b]' : 'bg-gray-200'
										} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
									>
										<span
											className={`${
												farcasterStates.frameData?.isCollection ? 'translate-x-6' : 'translate-x-1'
											} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
										/>{' '}
									</Switch>
								</div>
						...
						<div className={`${!farcasterStates.frameData?.isCollection && 'hidden'} mt-2`}>
							<InputBox
							...
							/>
							{farcasterStates.frameData?.isCollectionAddressError && <InputErrorMsg message={farcasterStates.frameData?.collectionAddressError} />}
						</div>
					</div>
				</div>
						...
						<div className="mb-4">
							<div className="flex justify-between">
								...
								<Switch
									...
									className={`${
										farcasterStates.frameData?.isExternalLink ? 'bg-[#e1f16b]' : 'bg-gray-200'
									} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
								>
									<span
										className={`${
											farcasterStates.frameData?.isExternalLink ? 'translate-x-6' : 'translate-x-1'
										} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
									/>{' '}
								</Switch>
							</div>
							<div className="w-4/5 opacity-75"> Let user know more about your frame. </div>

							<div className={`${!farcasterStates.frameData?.isExternalLink && 'hidden'} mt-2`}>
								<InputBox
									label="External Link"
									name="externalLink"
							...
							{farcasterStates.frameData?.isExternalLinkError && <InputErrorMsg message={farcasterStates.frameData?.isExternalLinkError} />}
						</div>
								...
								<Switch
									...
									onChange={() =>
										setFarcasterStates({
											...
											frameData: {
												...farcasterStates.frameData,
												isCustomCurrMint: !farcasterStates.frameData?.isCustomCurrMint,
												isCreatorSponsored: false,
											},
										})
									}
									className={`${
										farcasterStates.frameData?.isCustomCurrMint ? 'bg-[#e1f16b]' : 'bg-gray-200'
									} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
								>
									<span
										className={`${
											farcasterStates.frameData?.isCustomCurrMint ? 'translate-x-6' : 'translate-x-1'
										} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
									/>{' '}
								</Switch>
							</div>
							<div className="w-4/5 opacity-75"> Mint NFTs with custom currencies like $DEGEN </div>
						</div>
					</>
				)}
				<div className={`${!farcasterStates.frameData?.isCustomCurrMint && 'hidden'} mt-2`}>
					{isSponsoredChainFn() ? (
						<>
							...
							{LENSPOST_721_ENALBED_CHAINS?.map((network) => (
								...
								>
									<div className="flex items-center gap-1">
										<Avatar variant="circular" alt={network?.name} src={chainLogo(network?.id)} className="w-6 h-6" />
										<p>{network?.name}</p>
									</div>
								</Option>
							))}
					...
					<div className="my-2">
						<div className={`${!farcasterStates.frameData?.isCustomCurrMint && 'hidden'} `}>
							{farcasterStates?.frameData?.selectedNetwork?.name && (
								<>
									<div className="flex flex-row justify-between">
										...
										<div className="flex flex-col py-4 mx-2">
											...
											>
												{TOKEN_LIST[farcasterStates?.frameData?.selectedNetwork?.name]?.map((currency) => (
													<Option
														key={currency?.id}
														onClick={() => {
															setFarcasterStates({
													...
													>
														<div className="flex items-center gap-1">
															<Avatar variant="circular" alt={currency?.symbol} src={currency?.logoURI} className="w-6 h-6" />
															<p>{currency?.name}</p>
														</div>
													</Option>
												))}
									...
									{farcasterStates?.frameData?.isCustomCurrAmountError && <InputErrorMsg message={farcasterStates?.frameData.customCurrAmountError} />}
								</>
							)}
						</div>

						<div className={`flex flex-col w-full py-2 ${!farcasterStates.frameData?.isCustomCurrMint && 'hidden'}`}>
							<NumberInputBox
								min={1}
								step={1}
								label="Allowed Mints"
						...
						{farcasterStates.frameData?.isCustomCurrMint && farcasterStates.frameData?.allowedMints > walletData?.sponsored && isSponsoredChainFn() && (
							<Topup topUpAccount={walletData?.publicAddress} balance={walletData?.balance} refetchWallet={refetchWallet} sponsored={walletData?.sponsored} />
						)}
					</div>
				</div>
				{/* End */}
				<div className={`mb-4 ${!farcasterStates.frameData?.isCustomCurrMint && 'hidden'}`}>
					<h2 className="text-lg mb-2">Split Recipients</h2>
					<div className="flex justify-between">
						<Switch.Group>
							<Switch.Label className="w-4/5 opacity-60">Split revenue between multiple recipients</Switch.Label>
						</Switch.Group>
					</div>
					<div className="relative">
						{farcasterStates &&
							farcasterStates?.frameData?.fcSplitRevenueRecipients?.map((recipient, index) => {
								return (
									<div key={index} className="flex justify-between gap-2 items-center w-full py-2">
										<div className="flex-1">
											<InputBox
												label="Wallet Address"
												value={recipientsEns[index] || recipient.address}
												onFocus={(e) => restrictRecipientInput(e, index, recipient.address)}
												onChange={(e) => restrictRecipientInput(e, index, recipient.address)}
											/>
										</div>
										<div className="">
											<NumberInputBox
												...
												onFocus={(e) => {
													handleRecipientChange(index, 'percentAllocation', Number(parseFloat(e.target.value).toFixed(4)))
												}}
												onChange={(e) => {
													handleRecipientChange(index, 'percentAllocation', Number(parseFloat(e.target.value).toFixed(4)))
												}}
										...
										{!restrictRemoveRecipientInputBox(index, recipient.address) && (
											<span>
												<XCircleIcon
													...
													color="red"
													onClick={() => removeArrlistInputBox(index, 'fcSplitRevenueRecipients', 'isFcSplitError', 'fcSplitErrorMsg')}
												/>
											</span>
										)}
									</div>
						...
						<div className="flex justify-between items-center">
							{farcasterStates?.frameData?.isFcSplitError && (
								<>
									<InputErrorMsg message={farcasterStates?.frameData?.fcSplitErrorMsg} />
								</>
							)}
						</div>
								...
								<Switch
									...
									onChange={() =>
										setFarcasterStates({
											...
											frameData: {
												...farcasterStates.frameData,
												isCreatorSponsored: !farcasterStates.frameData?.isCreatorSponsored,
												isCustomCurrMint: false,
											},
										})
									}
									className={`${
										farcasterStates.frameData?.isCreatorSponsored ? 'bg-[#e1f16b]' : 'bg-gray-200'
									} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
								>
									<span
										className={`${
											farcasterStates.frameData?.isCreatorSponsored ? 'translate-x-6' : 'translate-x-1'
										} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
									/>{' '}
								</Switch>
							</div>
							<div className="w-4/5 opacity-75"> Let your audience mint your frame for free. </div>
						</div>
					)}
					{/* {actionType !== "composer" && ( */}
					<div className={`${!farcasterStates.frameData?.isCreatorSponsored && 'hidden'} mt-2`}>
						<div className="my-2">
							<p className="text-sm">
								{' '}
								{walletData?.sponsored > 0
									? `${walletData?.sponsored} mints are free. Topup with Base ETH if you want
              to drop more than ${walletData?.sponsored} mints ${' '}`
							...
							{farcasterStates.frameData?.isCreatorSponsored && farcasterStates.frameData?.allowedMints > walletData?.sponsored && (
								<Topup topUpAccount={walletData?.publicAddress} balance={walletData?.balance} refetchWallet={refetchWallet} sponsored={walletData?.sponsored} />
							)}
						</div>
					</div>
