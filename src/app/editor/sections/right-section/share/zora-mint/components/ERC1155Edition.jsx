...
import { useAccount, useChainId, usePublicClient, useWriteContract, useWalletClient, useSwitchChain } from 'wagmi'
import { createCreatorClient } from '@zoralabs/protocol-sdk'
import { ENVIRONMENT, getENSDomain, uploadJSONtoIPFS, uploadUserAssetToIPFS } from '../../../../../../../services'
...
const ERC1155Edition = () => {
	...
	const contractSymbol = zoraErc1155Enabled?.contractName?.split(' ')[0].toUpperCase()

	const { isLoading: isLoadingSwitchNetwork, switchChain } = useSwitchChain()
	const supportedChainId = ENVIRONMENT === 'production' ? base.id : baseSepolia.id
	...
	const isRecipientAddDuplicate = () => {
		const result = zoraErc1155Enabled.royaltySplitRecipients?.filter((item, index) => {
			return zoraErc1155Enabled.royaltySplitRecipients?.findIndex((item2) => item2.address.toLowerCase() === item.address.toLowerCase()) !== index
		})
	...
	const isPercentage100 = () => {
		const result = zoraErc1155Enabled.royaltySplitRecipients.reduce((acc, item) => {
	...
	const addArrlistInputBox = (key) => {
		if (key === 'royaltySplitRecipients') {
			setZoraErc1155Enabled({
				...zoraErc1155Enabled,
				[key]: [...zoraErc1155Enabled[key], { address: '', percentAllocation: '' }],
			})
			return
		}
	...
	const splitEvenPercentage = () => {
		const result = zoraErc1155Enabled.royaltySplitRecipients?.map((item) => {
			return {
				...
				percentAllocation: Math.floor((100 / zoraErc1155Enabled.royaltySplitRecipients.length).toFixed(2)),
			}
		})
		...
		<>
			...
			<div>
				<div className="flex flex-col w-full py-2">
					...
					/>
					{zoraErc1155StatesError.isContractNameError && <InputErrorMsg message={zoraErc1155StatesError.contractNameErrorMessage} />}
				</div>
			</div>
			...
			<div className={`${!zoraErc1155Enabled.isRoyaltySplits && 'hidden'}`}>
				<div className="mx-0">
					{zoraErc1155Enabled.royaltySplitRecipients?.map((recipient, index) => {
						return (
							<>
								<div key={index} className="flex justify-between gap-2 items-center w-full py-2">
									...
									<div className="flex justify-between items-center w-1/3">
										<NumberInputBox
											...
											onChange={(e) => {
												handleRecipientChange(index, 'percentAllocation', Number(parseFloat(e.target.value).toFixed(2)))
											}}
										/>
					...
					{zoraErc1155StatesError.isRoyaltySplitError && (
						<>
							<InputErrorMsg message={zoraErc1155StatesError.royaltySplitErrorMessage} />
			...
			<div className="mx-0 my-4">
				{actionType === 'composer' && !isAuthenticated ? (
					...
					<Button fullWidth disabled={isLoadingSwitchNetwork} onClick={() => switchChain({ chainId: supportedChainId })} color="red">
						{' '}
						Switch to Base network{' '}
					</Button>
				) : (
					<Button className="bg-[#e1f16b] text-black" fullWidth onClick={handleSubmit}>
						{' '}
						Create{' '}
					</Button>
				)}
