...
import { chainLogo, errorMessage, getFromLocalStorage, saveToLocalStorage } from '../../../../../../../utils'
import ZoraDialog from './ZoraDialog'
import { useCreateSplit } from '../../../../../../../hooks/0xsplit'
import { useMutation, useQuery } from '@tanstack/react-query'
import { EVMWallets } from '../../../../top-section/auth/wallets'
import { FarcasterAuth, FarcasterChannel } from '../../farcaster-share/components'
...
const ERC721Edition = ({ isOpenAction, isFarcaster, selectedChainId }) => {
	...
	const isUnsupportedChain = () => {
		...
		if (chainId === degen?.id || chainId === polygon?.id || chainId === ham?.id || chain?.unsupported || chain?.id != selectedChainId) return true
	}
	...
	const addArrlistInputBox = (key) => {
		if (key === 'royaltySplitRecipients') {
			setZoraErc721Enabled({
				...zoraErc721Enabled,
				[key]: [...zoraErc721Enabled[key], { address: '', percentAllocation: '' }],
			})
			return
		}
		...
		if (key === 'percentAllocation' && index === 0) {
			if (value < 10 || value > 100 || isNaN(value)) {
				setZoraErc721StatesError({
					...
					royaltySplitErrorMessage: 'Platform fee should be between 10% to 100%',
				})
			} else {
				setZoraErc721StatesError({
					...zoraErc721StatesError,
	...
	const handleChange = (e, index, key) => {
		...
		if (name === 'allowlistAddresses') {
			setZoraErc721Enabled((prevEnabled) => ({
				...prevEnabled,
				[name]: prevEnabled[name].map((item, i) => (i === index ? value : item)),
			}))
		} else {
	...
	const isRecipientAddDuplicate = () => {
		const result = zoraErc721Enabled.royaltySplitRecipients.filter((item, index) => {
			return zoraErc721Enabled.royaltySplitRecipients.findIndex((item2) => item2.address === item.address) !== index
		})
	...
	const isPercentage100 = () => {
		const result = zoraErc721Enabled.royaltySplitRecipients.reduce((acc, item) => {
	...
	const splitEvenPercentage = () => {
		const result = zoraErc721Enabled.royaltySplitRecipients.map((item) => {
			return {
				...
				percentAllocation: Math.floor((100 / zoraErc721Enabled.royaltySplitRecipients.length).toFixed(2)),
			}
		})
	...
	const handleMintSettings = () => {
		...
		if (zoraErc721Enabled.isChargeForMint) {
			publicSalePrice = (Number(zoraErc721Enabled.chargeForMintPrice) * 1e18).toString()
		}
		...
		if (zoraErc721Enabled.isPreSaleSchedule) {
			presaleStart = formatDateTimeUnix(zoraErc721Enabled.preSaleStartTimeStamp.date, zoraErc721Enabled.preSaleStartTimeStamp.time)
			presaleEnd = formatDateTimeUnix(zoraErc721Enabled.preSaleEndTimeStamp.date, zoraErc721Enabled.preSaleEndTimeStamp.time)
		}

		if (zoraErc721Enabled.isPublicSaleSchedule) {
			publicSaleStart = formatDateTimeUnix(zoraErc721Enabled.publicSaleStartTimeStamp.date, zoraErc721Enabled.publicSaleStartTimeStamp.time)
			publicSaleEnd = formatDateTimeUnix(zoraErc721Enabled.publicSaleEndTimeStamp.date, zoraErc721Enabled.publicSaleEndTimeStamp.time)
		}
	...
	return (
		<>
			...
			<div className=" mt-1 pt-2 pb-4">
				...
				<div className="w-4/5 opacity-75"> Set a Name and Symbol for the collection </div>
			</div>
			...
			<div className={` `}>
				<div className="flex flex-col w-full py-2">
					...
					/>
					{zoraErc721StatesError.isContractNameError && <InputErrorMsg message={zoraErc721StatesError.contractNameErrorMessage} />}
					<div className="mt-2">
						<InputBox
					...
					{zoraErc721StatesError.isContractSymbolError && <InputErrorMsg message={zoraErc721StatesError.contractSymbolErrorMessage} />}
				</div>
			</div>
			...
			<div className="mb-4 mt-4">
				<div className="flex justify-between">
					...
					>
						<span
							className={`${
								zoraErc721Enabled.isChargeForMint ? 'translate-x-6' : 'translate-x-1'
							} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
						/>{' '}
					</Switch>
				</div>
				<div className="w-4/5 opacity-75"> Set an amount to be charged for minting </div>
			</div>
			...
			<div className={`${!zoraErc721Enabled.isChargeForMint && 'hidden'}`}>
				...
				{zoraErc721StatesError.isChargeForMintError && <InputErrorMsg message={zoraErc721StatesError.chargeForMintErrorMessage} />}
			</div>
			...
			<div className="mb-4 mt-4">
				...
				<div className="w-4/5 opacity-75"> Split revenue between multiple recipients </div>
			</div>
				...
				<div className="">
								...
								{!restrictRemoveRecipientInputBox(index, recipient.address) && (
									<span>
										<XCircleIcon
											...
											color="red"
											onClick={() => removeArrlistInputBox(index, 'royaltySplitRecipients', 'isRoyaltySplitError', 'royaltySplitErrorMessage')}
										/>
									</span>
								)}
							</div>
					...
					<div className="flex justify-between items-center">
						{zoraErc721StatesError.isRoyaltySplitError && (
							<>
								<InputErrorMsg message={zoraErc721StatesError.royaltySplitErrorMessage} />
			...
			<>
				<div className="mb-4 mt-4">
					...
					<div className="w-4/5 opacity-75"> Set Royalty percentage for minting </div>
				</div>
				...
				<div className={`${!zoraErc721Enabled.isRoyaltyPercent && 'hidden'}`}>
					...
					{zoraErc721StatesError.isRoyaltyPercentError && <InputErrorMsg message={zoraErc721StatesError.royaltyPercentErrorMessage} />}
				</div>
			</>
			{/* Switch Number 5 End */}
			...
			<div className="mb-4 mt-4">
				<div className="flex justify-between">
					...
					<Switch
						...
						className={`${
							zoraErc721Enabled.isMintLimitPerAddress ? 'bg-[#e1f16b]' : 'bg-gray-200'
						} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
					>
						<span
							className={`${
								zoraErc721Enabled.isMintLimitPerAddress ? 'translate-x-6' : 'translate-x-1'
							} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
						/>{' '}
					</Switch>
				</div>
				<div className="w-4/5 opacity-75"> Limit the number of mints per address </div>
			</div>

			<div className={`${!zoraErc721Enabled.isMintLimitPerAddress && 'hidden'}`}>
				<div className="flex flex-col w-full py-2">
					<NumberInputBox
						min={'1'}
						step={'1'}
					...
					/>
					{zoraErc721StatesError.isMintLimitPerAddressError && <InputErrorMsg message={zoraErc721StatesError.limitedEditionErrorMessage} />}
				</div>
			</div>
			{/* Switch Number 3 End */}
			...
			<>
				<div className="mb-4 mt-4">
					<div className="flex justify-between">
						...
						>
							<span
								className={`${
									zoraErc721Enabled.isMaxSupply ? 'translate-x-6' : 'translate-x-1'
								} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
							/>{' '}
						</Switch>
					</div>
					<div className="w-4/5 opacity-75"> Set the maximum supply limit for the mint </div>
				</div>
				...
				<div className={`${!zoraErc721Enabled.isMaxSupply && 'hidden'}`}>
					...
					{zoraErc721StatesError.isMaxSupplyError && <InputErrorMsg message={zoraErc721StatesError.maxSupplyErrorMessage} />}
				</div>
			</>
			{/* Switch Number 6 End */}
			...
			<div className="mb-4 mt-4">
				...
				<div className="w-4/5 opacity-75"> Allow specific wallet addresses to mint </div>
			</div>
			...
			<div className={` ${!zoraErc721Enabled.isAllowlist && 'hidden'} `}>
					...
					return (
						<>
							<div key={index} className="flex justify-between gap-2 items-center w-full py-2">
								...
								<div className="flex justify-between items-center">
									{index != 0 && (
										<XCircleIcon className="h-6 w-6 cursor-pointer" color="red" onClick={() => removeArrlistInputBox(index, 'allowlistAddresses')} />
									)}
								</div>
							</div>
						</>
				...
				{zoraErc721StatesError.isAllowlistError && <InputErrorMsg message={zoraErc721StatesError.allowlistErrorMessage} />}
				<Button
					// color="yellow"
					size="sm"
					variant="filled"
				...
				<div className="mb-4 mt-4">
					<div className="flex justify-between">
						...
						<Switch
							...
							className={`${
								zoraErc721Enabled.isPreSaleSchedule ? 'bg-[#e1f16b]' : 'bg-gray-200'
							} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
						>
							<span
								className={`${
									zoraErc721Enabled.isPreSaleSchedule ? 'translate-x-6' : 'translate-x-1'
								} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
							/>{' '}
						</Switch>
					</div>
					<div className="w-4/5 opacity-75"> Set a start and end date for your Presale schedule of the mint </div>
				</div>

				<div className={`flex flex-col ${!zoraErc721Enabled.isPreSaleSchedule && 'hidden'} `}>
					<div className=" flex justify-between text-center align-middle">
						<div>Start</div> <DateTimePicker onChange={(e) => onCalChange(e, 'preSaleStartTimeStamp')} />
					</div>
					<div className="my-4 flex justify-between text-center align-middle">
						<div>End</div> <DateTimePicker onChange={(e) => onCalChange(e, 'preSaleEndTimeStamp')} />
					</div>

					{zoraErc721StatesError.isPreSaleScheduleError && <InputErrorMsg message={zoraErc721StatesError.presaleScheduleErrorMessage} />}
				</div>
			</>
			{/* Switch Number 7 End */}
				...
				<div className="mb-4 mt-4">
					<div className="flex justify-between">
						...
						<Switch
							...
							className={`${
								zoraErc721Enabled.isPublicSaleSchedule ? 'bg-[#e1f16b]' : 'bg-gray-200'
							} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#e1f16b] focus:ring-offset-2`}
						>
							<span
								className={`${
									zoraErc721Enabled.isPublicSaleSchedule ? 'translate-x-6' : 'translate-x-1'
								} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
							/>{' '}
						</Switch>
					</div>
					<div className="w-4/5 opacity-75"> Set a start and end date for your Public sale schedule of the mint </div>
				</div>

				<div className={`flex flex-col ${!zoraErc721Enabled.isPublicSaleSchedule && 'hidden'} `}>
					<div className="flex justify-between text-center align-middle">
						<div>Start</div> <DateTimePicker onChange={(e) => onCalChange(e, 'publicSaleStartTimeStamp')} />
					</div>
					<div className="my-4 flex justify-between text-center align-middle">
						<div>End</div> <DateTimePicker onChange={(e) => onCalChange(e, 'publicSaleEndTimeStamp')} />
					</div>

					{zoraErc721StatesError.isPublicSaleScheduleError && <InputErrorMsg message={zoraErc721StatesError.publicsaleScheduleErrorMessage} />}
				</div>
			...
			{!getEVMAuth ? (
				...
				<LensDispatcher title="Enable signless transactions" className="w-[95%] outline-none" />
				...
				<div className="outline-none">
					...
					>
						...
						{chains.find((i) => i.id === selectedChainId)?.name ? chains.find((i) => i.id === selectedChainId)?.name : 'a suported'} Network{' '}
