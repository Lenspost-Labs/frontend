...
const StoryMint = ({ selectedChainId }) => {
  ...
  const [storyTab, setStoryTab] = useState(
    isStoryNFT ? "registerDerivative" : "registerIP"
  );
  return (
    <>
      <SharePanelHeaders
        ...
        panelContent={
          <>
            <Tabs className="overflow-y-auto my-2" value={storyTab}>
              <TabsHeader>
                {isStoryNFT ? (
                  <Tab value="registerDerivative">Register Derivative</Tab>
                ) : (
                  <Tab value="registerIP">Register IP</Tab>
                )}
