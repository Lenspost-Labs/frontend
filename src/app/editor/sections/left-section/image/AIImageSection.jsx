...
import { Textarea, Button as MatButton, Select, Option } from '@material-tailwind/react'
...
const MIGGLES_QUERIES = ['Miggles as Superman', 'Miggles shaking hands with President Trump', 'Miggles takes over Japan', 'Miggles as Batman']
...
export const CompSearch = ({ StableDiffusionImages, isPanel = false, FluxImages, PepeImages, MigglesImages }) => {
		...
		<>
			<div className="">
				...
				<div className="flex flex-row justify-between items-center gap-2 w-full">
					<div
						className={`${
							model === 'MiggleV3' || model === 'PepeXL' || model === 'FLUX.1-dev' ? 'sm:grid w-full grid-cols-2 overflow-x-scroll' : 'overflow-auto'
						}`}
					>
						{(model === 'MiggleV3' ? MIGGLES_QUERIES : model === 'PepeXL' ? PEPE_QUERIES : model === 'FLUX.1-dev' ? FLUX_QUERIES : RANDOM_QUERIES2).map(
							(val, key) => {
								return (
									<div
										key={key}
					...
					{!isPanel && model !== 'MiggleV3' && model !== 'PepeXL' && model !== 'FLUX.1-dev' && (
...
const CompInstructImage = () => {
	...
	const [stImgPrompt, setStImgPrompt] = useState('A serene lakeside scene at sunset with vibrant orange and purple hues reflecting off the calm waters.')
	const [stDisplayMessage, setStDisplayMessage] = useState('Choose an Image & Click on GENERATE to customize image based on your prompt')
		...
		<>
			<div className="h-full overflow-y-auto">
				...
				<div className="mb-4 rounded-md">
					<input className="mb-2 ml-2" type="file" accept="image/*" onChange={handleImageUpload} />
					<div className="flex justify-center "> OR </div>
					<MatButton disabled={!fastPreview[0]} size="sm" color="deep-purple" variant="outlined" className="mt-4 p-2" fullWidth onClick={fnUseThisCanvas}>
						Use this Canvas
					</MatButton>
					{uploadedImg && (
				...
				<Textarea required color="deep-purple" label="Prompt" onChange={(e) => setStImgPrompt(e.target.value)} />

				<MatButton disabled={!uploadedImg} className="mt-4 w-full" onClick={fnCallInstructImgAPI}>
					Generate
					<img className="h-4 -mt-1 ml-2" src={coinImg} alt="" />
				</MatButton>

				{!responseImage && !clicked && <div className="mt-4 text-center text-md text-green-600">{stDisplayMessage}</div>}
...
const AIImagePanel = () => {
	...
	return (
		<div className="h-full">
			<Tabs id="custom-animation" className="overflow-y-auto h-full" value={currentTab}>
				<TabsHeader>
